...
from cruds.platform.telegram import (
    get_telegram_bots,
    add_telegram_bot,
    delete_telegram_bot,
)
...
@router.get(
    "/telegram", response_model=list[TelegramSchema], status_code=status.HTTP_200_OK
)
...
async def add_telegram_bot_(
    bot_token: str, chat_id: str, db_session=Depends(get_db_session)
):
...
async def delete_telegram_bot_(
    bot_token: str, chat_id: str, db_session=Depends(get_db_session)
):
    return delete_telegram_bot(
        bot_token=bot_token, chat_id=chat_id, db_session=db_session
    )
