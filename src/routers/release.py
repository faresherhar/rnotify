...
@router.get(
    "/unnotified", response_model=list[ReleaseSchema], status_code=status.HTTP_200_OK
)
...
@router.get(
    "/{provider}", response_model=list[ReleaseSchema], status_code=status.HTTP_200_OK
)
...
@router.get(
    "/{provider}/{owner}/{repo}/{tag}",
    response_model=ReleaseSchema,
    status_code=status.HTTP_200_OK,
)
async def get_release_(
    provider: str, owner: str, repo: str, tag: str, db_session=Depends(get_db_session)
):
    return get_release(
        provider=provider, owner=owner, repo=repo, tag=tag, db_session=db_session
    )
