...
def render_notification(provider: str, owner: str, repo: str, tag: str) -> str:
    template = Environment(loader=FileSystemLoader(notifier_settings.NOTIFICATION_TEMPLATES)).get_template("notification.txt")
...
def render_notification_markdown(provider: str, owner: str, repo: str, tag: str) -> str:
    template = Environment(loader=FileSystemLoader(notifier_settings.NOTIFICATION_TEMPLATES)).get_template("notification.md")
    return template.render(provider=provider.title(), owner=owner, repo=repo, tag=tag)
